...
pub fn get_details_by_trans(conn: &Connection, trans_id: i32) -> Result<Vec<Details>> {
    let mut stmt=conn.prepare("SELECT (id,trans_id,account_id,currency,balance) FROM DETAIL WHERE trans_id=?1")?;
...
pub fn get_details_by_account(conn: &Connection, account_id: i32) -> Result<Vec<Details>> {
    ...
    let mut stmt=conn.prepare("SELECT (id,trans_id,account_id,currency,balance) FROM DETAIL WHERE account_id=?1")?;
